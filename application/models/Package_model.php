...
class Package_model extends CI_Model {
    ...
    function list_featureData($currentID = NULL) {
      ...
      //-----------------------------------
    function addFeature($currentID = null,  $name = null,$nameth = null) {
        $data = array('include_name_th' => $nameth,
  ...
  $this->db->update($table, $data);
    ...
    return $data = 1;
     ...
     //=----------------
    function updateseason($currentID = null, $name = null,$nameth=null) {
        $data = array('include_name_th' => $nameth,
            'include_name_en' => $name);
     ...
     //=----------------
    function updateplace($currentID = null, $name = null , $nameth = null) {
        $data = array('place_name_th' => $nameth,
            'place_name_en' => $name);
    ...
    //--------------------------- 
    function addpackage($currentID= NULL, $name= NULL,$nameth = NULL, $desc= NULL,$descth = NULL,$include=null) {
        $data = array('package_name_en' => $name,
            'package_name_th' => $nameth,
            'package_detail' => $desc,
            'package_detail_th' => $descth,
            'data_status' => '1' );
        if ($currentID == '') {
    ...
    //--------------------------- 
    function addplace($currentID=null, $name=null,$nameth=null) {
    ...
    //-----------------------------------
    function addtransport($currentID= NULL, $name= NULL,$nameth=NULL, $comment= NULL,$commentth=NULL,$icon_class=null) {
        $data = array(
            'transport_name_en' => $name,
            'transport_name_th' => $nameth,
            'transport_info_en' => $comment,
            'transport_info_th' => $commentth,
   ...
   return $pass;
    ...
    function updatetransfer($currentID = null, $name = null) {
     ...
     function getlandList() {
		...
		$sql=$this->db->query("SELECT a.route_name_en, b.place_name_en,b.place_name_th  , a.id AS routeID , a.begin_place_id, a.destination_place_id  FROM  tbl_landtransfer AS a  LEFT  JOIN tbl_place_data AS b On a.begin_place_id=b.id  WHERE a.rout_active='1' GROUP BY a.begin_place_id");
    ...
    function placedataland($changeValue=null) {
       // $sql = $this->db->query("SELECT d.destination_place_id , p.* FROM `tbl_route_data` AS d LEFT JOIN tbl_place_data AS p ON d.destination_place_id = p.id    WHERE d.show_onweb = '1' AND d.rout_active = '1' AND d.begin_place_id = '".$changeValue."' ");
		...
		$sql=$this->db->query("SELECT a.route_name_en, b.place_name_en ,b.place_name_th , a.id AS routeID , a.begin_place_id, a.destination_place_id  FROM  tbl_landtransfer AS a  LEFT  JOIN tbl_place_data AS b On a.destination_place_id=b.id  WHERE a.rout_active='1' AND a.begin_place_id = '".$changeValue."'  GROUP BY a.destination_place_id  ");
        return $sql;
    }
       //---------------------------
